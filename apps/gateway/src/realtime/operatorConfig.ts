...
export const ACTION_REALTIME_ROOM_TOOLS: RealtimeFunctionTool[] = REALTIME_ROOM_TOOLS;
...
export function buildOperatorInstructions(): string {
  return [
    ...
    "This operator is called only after a deterministic prefilter selected a map-worthy turn. Make one useful state update when the turn is complete enough; otherwise use ignore_turn.",
    ...
    "If a real center already exists, never replace it with a generic clarification like 'clarify the main question' or 'define the topic'. Use correct_map_node only when the transcript explicitly gives the corrected title/summary.",
    ...
    "Filler, acknowledgements, jokes, repetition, partial sentences, abandoned starts, or turns too small to preserve should not reach this operator. If one does, use ignore_turn unless it contains a complete useful correction.",
